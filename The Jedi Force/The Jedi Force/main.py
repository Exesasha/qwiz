...
volume = 1  # Громкость звука в целом(фоновая музыка и звуки) (1 - включен, 0 - выключен)
volume1 = 1  # Переменная для выключения и включения фоновой музыки
...
def write_text(type1, content, size, color, x, y, transfer_type=0, options=0):
    ...
    if transfer_type != 0:
        ...
        for i in result:
            ...
            screen.blit(text, (text_rect.x, y + a * 50))
            a += 1
    ...
    if object1.collidepoint(mouse_position):
        if mouse_pressed[0]:
            ...
            if content == 'ВЫХОД':
                ...
                # Если нажата кнопка с правильным вариантом ответа, то добавляется очко и кнопка становится зелёной и
                # играет звук правильного ответ
                if content == answer:
                    score += 1
                    ...
                    if volume == 1:
                        right.play()
                # Если нажата кнопка с неправильным вариантом ответа, то кнопка становится красной и играет звук
                # неправильного ответ
                else:
            ...
            # Если курсор на кнопке и при этом нет нажатия мыши, то цвет кнопки меняется
...
def questions_list(subject):
    ...
    for i in range(0, 10, 1):
        random1 = random.randint(0, len(data1) - 1)  # Случайный выбор списка вопросов
...
def choose_subject_screen():
    ...
    for i in range(0, len(data), 1):
        # В каждом стобцу одинаковое значение по x, в каждой строке - по y
        if 1 <= i + 1 <= 5:
            y = 250
        elif 6 <= i + 1 <= 10:
            y = 450
...
while True:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            sys.exit() # Стандартная обработка нажатия крестика в окне

    # Отображение текущего экрана в зависимости от значения переменной transition            
    if transition == 0:
        start_screen() # Начальный экран
    elif transition == 1:
        choose_subject_screen() # Экран выбора предметов
    elif transition == 2:
        settings_screen() # Экран настроек
    elif transition == 3:
        questions() # Экран вопроса
    elif transition == 4:
        results(score)  # Экран результатов
